...
const MatchOverlay = ({ user, onClose, onMessage }) => (
    ...
    >
        ...
        >
            <h1 className="text-5xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-rose-500 to-indigo-600 mb-8 drop-shadow-[0_0_10px_rgba(244,63,94,0.2)]">
                IT'S A MATCH!
            </h1>
...
const Discover = () => {
    const { userProfile, discoverQueue, setDiscoverQueue, matchedUsers, setMatchedUsers, setActiveTab, setActiveChat } = useUser();
    const [swipeCount, setSwipeCount] = useState(0);
    const [currentMatch, setCurrentMatch] = useState(null);

    // Filter queue by preference
    const preference = userProfile?.basics?.['Interested in'] || 'Everyone';
    const filteredQueue = discoverQueue.filter(u => preference === 'Everyone' || u.gender === preference);

    const handleSwipe = (direction, id) => {
        // Find the user being swiped
        ...
        if (direction === 'right' || direction === 'super') {
            ...
            // Deterministic Match Logic: Every 3rd right swipe, OR instantly on SuperSwipe
            if (newCount % 3 === 0 || direction === 'super') {
                setCurrentMatch(swipedUser);
                
                // Add to matchedUsers Context (with mock welcome message)
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col relative bg-slate-50 transition-colors duration-300">
