...
const ChatList = ({ setView, setActiveChat }) => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        ...
        <div className="px-6 py-4 flex gap-4 overflow-x-auto no-scrollbar border-b border-slate-100 dark:border-slate-800">
            <div className="flex flex-col items-center gap-2 cursor-pointer group">
                <div className="w-16 h-16 rounded-full border-2 border-indigo-500 flex items-center justify-center bg-indigo-50 dark:bg-indigo-900/30 group-hover:scale-105 transition">
            ...
            {CHATS.map((chat) => (
                <div 
                    ...
                    className="flex flex-col items-center gap-2 cursor-pointer hover:opacity-80 transition group"
                >
                    <div className="w-16 h-16 rounded-full border-2 border-transparent p-0.5 bg-gradient-to-tr from-indigo-500 to-rose-400 group-hover:scale-105 transition">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-2 pb-6">
            ...
            {CHATS.map((chat) => (
                <div 
                    ...
                    className="flex items-center gap-4 p-3 hover:bg-white dark:hover:bg-slate-900 rounded-2xl transition cursor-pointer border border-transparent hover:border-slate-100 dark:hover:border-slate-800 hover:shadow-sm group"
                >
                    <img src={chat.img} alt={chat.name} className="w-14 h-14 rounded-full object-cover shadow-sm group-hover:scale-105 transition" />
