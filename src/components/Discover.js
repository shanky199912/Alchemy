            ...
            <div className="flex-1 relative flex items-center justify-center overflow-hidden w-full pb-4">
                ...
                {cards.length > 0 && (
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center justify-center gap-5 z-20 bg-white/70 backdrop-blur-md px-5 py-2.5 rounded-full shadow-lg border border-white/40">
                        <button onClick={() => cards.length && handleSwipe(cards[0].id)} className="w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-rose-500 hover:bg-rose-50 transition border border-slate-100">
                            <X className="w-5 h-5" strokeWidth={3} />
                        </button>
                        <button onClick={() => cards.length && handleSwipe(cards[0].id)} className="w-[34px] h-[34px] bg-white rounded-full shadow-md flex items-center justify-center text-indigo-500 hover:bg-indigo-50 transition border border-slate-100">
                            <Star className="w-4 h-4 fill-indigo-500" />
                        </button>
                        <button onClick={() => cards.length && handleSwipe(cards[0].id)} className="w-10 h-10 bg-gradient-to-tr from-indigo-500 to-purple-500 rounded-full shadow-md flex items-center justify-center text-white hover:opacity-90 transition">
